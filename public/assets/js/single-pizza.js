...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      ...
      // if no id or pizza can't be found, send error
      if (!response.ok) {
    ...
    .then(printPizza)
    // catch so that any error takes user back to homepage, using window.history.back() method
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza with this id! Taking you back.');
      // as long as the browser session has previous page, it will behave as if user had clicked the "back" button
      window.history.back();
    })
}
